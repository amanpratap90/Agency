...
export default function HomeClient() {
    ...
    const openUserAuth = () => setIsUserAuthModalOpen(true);
                    ...
                    <nav className="text-sm text-neutral-400 gap-8 hidden md:flex items-center bg-white/5 px-6 py-3 rounded-full border border-white/5 backdrop-blur-sm">
                        ...
                        {user ? (
                            <div className="flex items-center gap-4">
                                ...
                                <Link href="/my-account" className="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-600 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-orange-500/20 transition-all">My Account</Link>
                            </div>
                        ) : (
                        ...
                        {isMenuOpen && (
                            ...
                            >
                                <nav className="flex flex-col gap-8 text-2xl font-bold text-center">
                                    ...
                                    {user ? (
                                        <Link href="/my-account" onClick={closeMenu} className="text-orange-500 hover:text-orange-400 transition-colors">My Account</Link>
                                    ) : (
